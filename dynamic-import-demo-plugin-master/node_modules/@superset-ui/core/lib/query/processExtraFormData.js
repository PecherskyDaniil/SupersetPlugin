...
exports.appendExtraFormData = appendExtraFormData;
...
function overrideExtraFormData(queryObject, overrideFormData) {
  const overriddenFormData = _extends({}, queryObject);
...
function appendExtraFormData(queryObject, appendFormData) {
  const appendedFormData = _extends({}, queryObject);

  ALLOWED_APPENDS.forEach(key => {
    if (key in appendFormData) {
      const append = appendFormData[key];
      const currentValue = appendedFormData[key] || []; // @ts-ignore
